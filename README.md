...
단순 CRUD가 아니라 다음을 한 프로젝트에서 보여줍니다.

- Rust 서비스(Papermake)와 NestJS 연동
- 비동기 큐, 재시도, DLQ
- 멱등성(Idempotency) 설계
- S3 객체 저장 + Signed URL
- ClickHouse 이벤트 분석
- 멀티테넌트 SaaS + API Key/Webhook 보안
- 템플릿 버전 관리, 승인 워크플로
- PDF/A 전자문서, 대량 처리, 장애 복구
- 감사 로그와 데이터 **재현(reproducibility)**

카카오, 네이버 전자문서 발송 연동 경험과 연결하면 실무 설득력이 높습니다.
