...
- 렌더 → PDF/A 변환 → S3 저장 → 해시 계산 → 상태 갱신 → 증적/통계 기록 → Webhook 트리거.
...
[Worker] 처리
  ...
  14. 렌더 결과를 통계 집계용으로 기록 (PostgreSQL)
  15. Webhook 발송 (서명 포함)
...
두 저장소가 각자의 강점에 맞는 데이터만 담는다.

### PostgreSQL — 트랜잭션/상태/증적/통계
...
- Webhook 엔드포인트/시크릿
- 권한(RBAC)
- 문서 증적(template/input/output hash, 타임스탬프)
- 렌더 통계 집계(성공률, 처리시간, 처리량, 오류코드)
- 과금 정보(스키마만 선행)
...
> Papermake도 기본적으로 S3 계열 저장소를 사용하므로 객체 저장 인프라가 자연스럽게 정렬된다.

## 2.5 왜 이 경계인가

- **상태와 증적을 한 곳(PostgreSQL)에서 강하게 일관 관리**: 문서 상태 전이와 증적 기록이 같은 트랜잭션 경계 안에 있어 정합성이 높다.
- **통계는 별도 OLAP 없이 시작**: 초기에는 `document` 테이블 기반 집계(인덱스/집계 테이블)로 충분하다. 트래픽이 커지면 그때 분석 전용 저장소 도입을 재검토한다.
...
| 환경     | 구성                                                                           |
| -------- | ------------------------------------------------------------------------------ |
| 로컬     | docker-compose: NestJS + Worker + Papermake + MinIO + Postgres + Redis(BullMQ) |
| 클라우드 | ECS/Fargate(API, Worker) + SQS + S3 + RDS(Postgres) + Papermake 컨테이너       |
