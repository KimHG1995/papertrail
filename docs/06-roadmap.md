# 06. 로드맵, 마일스톤

## 6.1 최종 추천 범위

처음에는 아래 4대 흐름만 완성한다.

```
템플릿 등록
  → JSON 데이터 입력
  → 비동기 PDF 생성
  → 생성 이력 및 PDF 다운로드
```

그다음 차별화 기능을 추가한다.

```
CSV 대량 생성
Webhook
PDF/A
문서 재현
템플릿 승인
카카오, 네이버 전자문서 발송 연동
```

---

## 6.2 마일스톤

### M0 — 명세 (현재)

- [x] 제품/기술 명세서 정의
- [x] 리포지토리, 문서 구조
- [x] docker-compose 로컬 인프라 초안(Papermake, MinIO, ClickHouse, Postgres, Redis)

### M1 — MVP 코어 (4대 흐름)

- [x] NestJS 게이트웨이 스캐폴딩 + API Key 인증/테넌트 격리(전역 AuthGuard, 스코프)
- [x] Papermake 연동 클라이언트(publish/render/download 래핑, http + 로컬 fake 드라이버)
- [x] 템플릿 등록(publish) + JSON Schema 등록/입력 검증 + 태그 관리
- [x] `POST /v1/documents` 단건 + 멱등성 + 입력 스키마 검증
- [x] 큐(BullMQ 로컬, SQS 추상화 가능) + Render Worker
- [x] 재시도/DLQ + 테넌트별 동시성 제한(Redis 정렬셋 + 리스 분산 세마포어)
- [x] 증적 기록(template/input/output hash) + 문서 상세 API
- [x] Signed URL 다운로드(결과 PDF S3/MinIO 저장 포함)
- **완료 정의:** [01. 기능 명세 §8](01-spec.md) 수용 기준 통과

### M2 — 차별화

- [x] CSV 대량(batch) 생성 + 진행률 + 결과 리포트(CSV, Signed URL)
- [x] Webhook 서명 발송 + 재시도(HMAC-SHA256, 엔드포인트 등록, delivery 추적)
- [ ] PDF/A-2b / a-3b 옵션
- [x] 템플릿 승인 워크플로(DRAFT→REVIEWING→APPROVED→PUBLISHED→DEPRECATED, PUBLISHED 만 렌더) + 스코프 기반 승인자 권한(templates:approve). 콘솔 사용자 RBAC 는 M3.
- [~] 미리보기(POST /v1/templates/{name}/preview, 큐/발행 게이트 우회 동기 렌더 → Signed URL) 완료, 버전 diff 예정
- [x] ClickHouse 이벤트 적재 + 통계 API(render_event 적재, /v1/stats/overview)

### M3 — 운영/관측/보안 강화

- [~] Admin 콘솔(Next.js) — 대시보드(사용량/통계), 템플릿 목록, 감사 로그(서버 렌더, 계약 타입 재사용) 완료, 나머지 관리 화면 예정
- [~] PII 마스킹(maskedPreview) + 입력 원문 암호화 저장(AES-256-GCM, storeInput 옵트인 → 서버 단독 재현 검증) 완료, 보관정책(lifecycle) 예정
- [x] 테넌트별 레이트 리밋(429 + Retry-After) + 월 렌더 쿼터(GET /v1/usage, 429 QUOTA_EXCEEDED) + 운영 감사 로그(변경 요청 자동 기록, GET /v1/audit)
- [x] 재현 검증 도구(POST /v1/documents/{id}/verify, 동일 입력 재렌더 → outputHash 대조)
- [~] 관측성: Prometheus 메트릭(/metrics, HTTP RED) + 분산 트레이싱(OpenTelemetry, 게이트웨이→큐→워커 트레이스 컨텍스트 전파) 완료, 알림 예정

### M4 — 전자문서 발송 연동

- [ ] 카카오 전자문서 발송 연동
- [ ] 네이버 전자문서 발송 연동
- [ ] 발송 결과 리포트/상태 콜백

---

## 6.3 우선순위 원칙

1. **핵심 차별화(증적/재현/멱등)를 M1에서 먼저 증명.** 단순 렌더는 Papermake가 이미 함.
2. **큐/워커는 추상화**해서 로컬(BullMQ)과 클라우드(SQS)를 같은 코드로.
3. **보안/멀티테넌트는 처음부터** 미들웨어에 심는다(나중 이식은 비용 큼).
4. Admin UI는 API가 안정된 뒤 M3에서 집중.

## 6.4 리스크 / 열린 질문

| 항목                 | 내용                                                                     |
| -------------------- | ------------------------------------------------------------------------ |
| Papermake API 안정성 | 스텁/실험 단계 기능이 있어 버전 pin + 통합 테스트 필요                   |
| 재현성 정밀도        | 폰트/렌더러 버전 차이로 `outputHash`가 흔들릴 수 있음 → 렌더러 버전 고정 |
| 입력 정규화 규칙     | `inputHash` 안정성의 핵심. 초기에 확정하고 문서화                        |
| 큐 전환 비용         | BullMQ ↔ SQS 시맨틱 차이(가시성 타임아웃 등) 흡수                        |
| PII 정책             | 테넌트별 보관/삭제 요구가 다를 수 있음 → 정책 파라미터화                 |

## 참고

- 상위 개요 → [00. 개요](00-overview.md)
- Papermake 원본 → <https://github.com/rkstgr/papermake>
